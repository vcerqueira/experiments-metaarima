...
X, y, _, _, _ = mdr.read(from_dev_set=True, fill_na_value=-1, max_config=ORDER_MAX_NONSEASONAL)
...
for variant_ in meta_arima_d:
    ...
    meta_arima_d[variant_].meta_fit(X_, y_)
...
for uid in uids:
    ...
    try:
        ...
        fcst_aa['ds'] = df_uid_ts['ds']
        fcst_meta_arima['ds'] = df_uid_ts['ds']
...
results_df.to_csv(f'assets/results/sensitivity/ablation,{target}.csv', index=False)
