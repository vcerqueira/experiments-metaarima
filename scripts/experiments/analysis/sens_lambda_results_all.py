...
def load_lambda_results():
    pattern = os.path.join(SENSITIVITY_DIR, '*lambda*csv')
    ...
    for path in paths:
        ...
        dataset_name = dataset_name.replace('lambda,', '').replace('.csv', '')

        df_meta = pd.DataFrame(
            {
                'lambda': lambdas.values,
                'MASE': avg_scores[meta_mask].values,
                'dataset': dataset_name,
            }
        )

        # Lower MASE is better: compute rank within each dataset
...
df_meta_all = load_lambda_results()

avg_ranks = (
    df_meta_all.groupby('lambda', as_index=False)['rank']
    .mean()
...
plot = (
        p9.ggplot(avg_ranks, p9.aes(x='lambda', y='avg_rank'))
        + THEME
        + p9.theme(
    plot_margin=0.015,
...
#     mapping=p9.aes(x='lambda', y='avg_rank', group=1),
#     method='loess',
#     se=False,
#     colour='orangered',
#     size=1.2,
...
plot = plot + p9.xlab('λ') + p9.ylab('Average MASE rank')

plot.save(PLOT_NAME, width=11, height=4)
