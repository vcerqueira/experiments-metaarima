...
results_df = pd.read_csv('assets/results/sensitivity/quantile,monash_m3_monthly.csv')

avg_scores = results_df.mean(numeric_only=True)

meta_arima_mask = avg_scores.index.str.contains('MetaARIMA')
df_meta = pd.DataFrame({
    'quantile': avg_scores.index[meta_arima_mask].str.extract(r'\((.*?)\)')[0].astype(float),
...
plot = p9.ggplot(df_meta, p9.aes(**{'x': 'quantile', 'y': 'MASE'})) + \
       THEME + \
