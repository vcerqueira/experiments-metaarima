...
class _HalvingMetaARIMABase2(_MetaARIMABase):
    def __init__(self,
                 config_space: List[str],
                 season_length: int,
                 freq: str,
    ...
    def _evaluate_models_batch(self, df: pd.DataFrame, model_indices: List[int], sample_size: int) -> List[Tuple[int, float]]:
        """
        Evaluate models in a single batch to reduce overhead.
        ...
        # Create single StatsForecast instance for all models
        sf_subset = StatsForecast(models=models_subset, freq=self.freq)
        sf_subset.fit(df=df_subset)

        # Extract AICc scores in vectorized way
        aicc_scores = np.array([fit.model_['aicc'] for fit in sf_subset.fitted_[0]])
        
        return list(zip(model_indices, aicc_scores))

    def fit(self, df: pd.DataFrame):
        """
        Optimized fit using successive halving with batched evaluation.
        """
        n_rows = df.shape[0]
        n_models = self.nmodels
        s_max = int(log(n_models, self.eta))

        # Pre-calculate sample sizes for all rounds
        min_sample_size = self.season_length * self.init_resource_factor if self.season_length > 1 else 20
        sample_sizes = [min(n_rows, int(min_sample_size * (self.resource_factor ** s))) for s in range(s_max + 1)]

        # Initialize remaining indices as numpy array for faster operations
        remaining_indices = np.arange(n_models)

        self.tot_nobs = 0
        for s, sample_size in enumerate(sample_sizes):
            # Batch evaluate all remaining models
            eval_results = self._evaluate_models_batch(df, remaining_indices.tolist(), sample_size)
            self.tot_nobs += len(remaining_indices) * sample_size

            # Sort and select top models efficiently
            sorted_results = sorted(eval_results, key=lambda x: x[1])
            n_keep = max(int(n_models / (self.eta ** (s + 1))), self.min_configs)
            
            remaining_indices = np.array([idx for idx, _ in sorted_results[:n_keep]])
